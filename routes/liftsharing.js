module.exports = function(router, db, apiToken, querystring) {
  router.get('/liftsharing/:id', function(req, res, next) {
 		if(req.session.userid == req.params.id){ 
 			db.all("select club_name from club left join join_club on club.club_id = join_club.club_holder_id where holder_id = ?", [req.params.id], function(err, rows) {
 			  if (err) {
