module.exports = function(router, db, apiToken, querystring) {
  ...
  router.get('/liftsharing/:id/:club_id/mylifts/:route_id/editlift', function(req, res, next) {
 		if(req.session.userid == req.params.id){ 
 			db.all("SELECT * FROM route WHERE id = ?", [req.params.route_id], function(err, rows){	
        if(err) {
  ...
  router.post('/liftsharing/:id/:club_id/mylifts/:route_id/editlift', function(req, res, next) {
   response = {
    ...
    //   db.run("UPDATE route SET drop_off_location = ? WHERE id = ?", [response.drop_off_location, req.params.route_id], function(err, result){ 
      db.run("UPDATE route SET pick_up_location = ?, pick_up_time = ?, pick_up_date = ?, drop_off_location = ?, drop_off_time = ?, drop_off_date = ?, seats = ?, return_trip = ? WHERE id = ?", [response.pick_up_location, response.pick_up_time, response.pick_up_date, response.drop_off_location, response.drop_off_time, response.drop_off_date, response.seats, response.return_trip, req.params.route_id], function(err, result){ 
        if (err) { 
          return next(err); 
        }
